...
function CrudIngredientes() {
  ...
  const fetchIngredientes = async () => {
    try {
      const response = await axios.get(API_ENDPOINTS.INGREDIENTES);
  ...
  const crearIngrediente = async () => {
    try {
      await axios.post(API_ENDPOINTS.INGREDIENTES, {
        nombre,
        descripcion,
        precio,
      });
  ...
  const actualizarIngrediente = async () => {
    try {
      await axios.put(API_ENDPOINTS.INGREDIENTE_BY_ID(editingId), {
        nombre,
        descripcion,
        precio,
      });
  ...
  const eliminarIngrediente = async (id) => {
    try {
      await axios.delete(API_ENDPOINTS.INGREDIENTE_BY_ID(id));
      fetchIngredientes();
    } catch (error) {
