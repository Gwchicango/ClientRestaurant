...
const Orden = () => {
  ...
  const fetchOrdenesActuales = async () => {
    try {
      const response = await axios.get(API_ENDPOINTS.ORDENES_COCINA);
  ...
  const handleCantidadPlato = (platoId, ordenId, estado) => {
    ...
    if (estado === "Aumentar") {
      if (platoEnOrden) {
        ...
        axios.put(API_ENDPOINTS.PEDIDO_CANTIDAD(platoId, ordenId), updateData)
          .then((response) => {
        ...
        axios.put(API_ENDPOINTS.ORDEN_ESTADO(ordenId), updateOrden)
          .then((response) => {
      ...
      if (platoEnOrden) {
        ...
        if (numeroPlatosAntiguos !== platoEnOrdenActual.CANTIDAD_PLATOS_PEDIDOS) {
          if (platoEnOrdenActual.CANTIDAD_PLATOS_PEDIDOS > 1) {
            ...
            axios.delete(API_ENDPOINTS.PEDIDO_DELETE(platoId, ordenId))
              .then((response) => {
                console.log("Pedido updated successfully:", response.data);
                // You can also update the UI or state here if needed
              })
  ...
  const verificarCedulaExistente = (cedula) => {
    ...
    return axios
      .get(API_ENDPOINTS.CLIENTE_BY_CEDULA(cedula))
  ...
  const handleAgregarCliente = async (event) => {
    ...
    try {
      ...
      console.log(dataToSend)
      await axios.post(API_ENDPOINTS.CLIENTES, dataToSend);
  ...
  const verificarClienteConsumidorFinal = async () => {
    try {
      ...
      if (!existingClientData) {
        ...
        await axios.post(API_ENDPOINTS.CLIENTES, {
          CEDULA_CLI: "9999999999",
          NOMBRE_CLI: "CONSUMIDOR FINAL",
          DIRECCION_CLI: "",
          TELEFONO_CLI: "",
  ...
  const enviarPedidos = async () => {
    ...
    orden.forEach((pedido) => {
      ...
      axios
        .post(API_ENDPOINTS.PEDIDOS, dataToSend)
        .then((response) => {
    ...
    try {
      await axios
        .put(API_ENDPOINTS.ORDEN_BY_ID(IDordenActual), dataToUpdate)
        .then((response) => {
  ...
  const ObtenerOrden = (idOrden) => {
    ...
    if (ordenEscogida) {
      ...
      axios
        .get(API_ENDPOINTS.ORDEN_COCINA_BY_ID(idOrden))
        .then((response) => {
  ...
  const handleEditarPlatos = (idPlato) => {
    ...
    if (platoSeleccionado) {
      ...
      axios.post(API_ENDPOINTS.PEDIDOS_NUEVOS, updateData)
        .then((response) => {
          console.log("Pedido updated successfully:", response.data);
          // You can also update the UI or state here if needed
        })
  ...
  const handleDomicilioSelection = (selectedValue, idPlato, idOrden) => {
    ...
    if (platoSeleccionado) {
      ...
      if (platoEnOrdenActual) {
        ...
        axios.put(API_ENDPOINTS.PEDIDO_DOMICILIO(idPlato, idOrden), updateData)
          .then((response) => {
            console.log("Pedido updated successfully:", response.data);
            // You can also update the UI or state here if needed
          })
              ...
              {categoria === categoriaSeleccionada && (<>
                <ul>
                  {platosFiltrados.map((plato) => (
                    <li key={plato.ID_PL}>
                      <img src={`${API_ENDPOINTS.UPLOADS}${plato.FOTO_PL}`} alt={plato.NOMBRE_PL} />
