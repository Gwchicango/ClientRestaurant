...
const CrudPlatos = () => {
  ...
  const handleActualizarPlato = (id) => {
    ...
    if (
      ...
      axios
        .put(API_ENDPOINTS.PLATO_BY_ID(id), formData)
        .then((response) => {
  ...
  const handleAgregarPlato = () => {
    ...
    axios
      .post(API_ENDPOINTS.PLATOS, formData)
      .then((response) => {
        ...
        axios
          .get(API_ENDPOINTS.INGREDIENTE_BY_NOMBRE(nombreIngredienteSeleccionado))
          .then((response) => {
            ...
            axios
              .post(API_ENDPOINTS.RECETAS, nuevaReceta)
              .then(() => {
  ...
  const handleActualizarEstadoPlato = (idPlato, estadoActual) => {
    ...
    axios
      .put(API_ENDPOINTS.PLATO_ESTADO(idPlato), { Estado: nuevoEstado })
      .then((response) => {
    ...
    <div className="Crud">
      ...
      <ul>
        ...
        {platosDeshabilitados.map((plato) => (
              ...
              <img src={`${API_ENDPOINTS.UPLOADS}${plato.FOTO_PL}`} alt={plato.NOMBRE_PL}  style={{filter: "grayscale(1)"}}/>
            <div className="botonesCrud">
              <button onClick={() => handleEditarPlato(plato)}>Editar</button>
              <button onClick={() => handleActualizarEstadoPlato(plato.ID_PL, plato.ESTADO_PL)}>
                {plato.ESTADO_PL === 1 ? "Deshabilitar" : "Habilitar"} Plato
